...
Examples:
  ...
  dittofsctl store metadata add --name persistent-meta --type badger --db-path /data/meta
  ...
  dittofsctl store metadata add --name pg-meta --type postgres --config '{"host":"localhost","dbname":"dittofs"}'

  # Add a PostgreSQL store interactively
  dittofsctl store metadata add --name pg-meta --type postgres`,
	RunE: runAdd,
}
...
func buildMetadataConfig(storeType, jsonConfig, dbPath string) (any, error) {
	...
	case "postgres":
		...
		port, err := prompt.Input("PostgreSQL port", "5432")
		if err != nil {
			return nil, err
		}
