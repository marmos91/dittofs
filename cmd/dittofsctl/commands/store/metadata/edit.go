...
Examples:
  ...
  dittofsctl store metadata edit default

  # Update config with JSON
  dittofsctl store metadata edit default --config '{"db_path":"/new/path"}'

  # Update type
  dittofsctl store metadata edit default --type badger

  # Update BadgerDB path
  dittofsctl store metadata edit default --db-path /new/path`,
