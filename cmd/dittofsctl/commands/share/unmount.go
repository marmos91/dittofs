...
Examples:
  # Unmount a share
  dittofsctl share unmount /mnt/dittofs

  # Force unmount if busy
  dittofsctl share unmount --force /mnt/dittofs

Note: Unmount commands typically require sudo/root privileges.`,
	Args: func(cmd *cobra.Command, args []string) error {
		if len(args) < 1 {
			return fmt.Errorf("requires mount point path\n\nUsage: dittofsctl share unmount [mountpoint]\n\nExample: dittofsctl share unmount /mnt/dittofs")
		}
		if len(args) > 1 {
...
func runUnmount(cmd *cobra.Command, args []string) error {
	...
	if os.Geteuid() != 0 {
		return fmt.Errorf("unmount requires root privileges\nHint: Run with sudo: sudo dittofsctl share unmount %s", mountPoint)
	}
