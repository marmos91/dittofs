...
Examples:
  # Mount via NFS
  dittofsctl share mount --protocol nfs /export /mnt/dittofs

  # Mount via SMB
  dittofsctl share mount --protocol smb /export /mnt/dittofs

  # Mount via SMB with explicit credentials
  dittofsctl share mount --protocol smb --username alice /export /mnt/dittofs

  # Mount via SMB with password from environment
  DITTOFS_PASSWORD=secret dittofsctl share mount --protocol smb /export /mnt/dittofs

  # Mount to user directory without sudo (macOS only, recommended)
  mkdir -p ~/mnt/dittofs && dittofsctl share mount --protocol smb /export ~/mnt/dittofs
...
Platform differences for SMB with sudo:
  ...
  - macOS alternative: mount to ~/mnt without sudo for user-owned mount`,
	Args: func(cmd *cobra.Command, args []string) error {
		if len(args) < 2 {
			return fmt.Errorf("requires share path and mount point\n\nUsage: dittofsctl share mount [share] [mountpoint] --protocol <nfs|smb>\n\nExample: dittofsctl share mount --protocol nfs /export /mnt/dittofs")
		}
		if len(args) > 2 {
...
func checkMountPrivileges(mountPoint, protocol, sharePath string) error {
	...
	hint := fmt.Sprintf("Run with sudo: sudo dittofsctl share mount --protocol %s %s %s", protocol, sharePath, mountPoint)
	if runtime.GOOS == "darwin" {
		hint += fmt.Sprintf("\nOr mount to your home directory: dittofsctl share mount --protocol %s %s ~/mnt/share", protocol, sharePath)
	}
...
func resolveSMBUsername() (string, error) {
	...
	return "", fmt.Errorf("username required for SMB mount\nHint: Use --username flag or login with 'dittofsctl login'")
}
...
func formatMountError(err error, output, protocol string, port int) error {
	...
	if strings.Contains(outputLower, "connection refused") || strings.Contains(errStr, "connection refused") {
		return fmt.Errorf("mount failed: %w\nOutput: %s\nHint: Is the %s adapter running? Check with 'dittofsctl adapter list'", err, output, protocol)
	}
	...
	if strings.Contains(outputLower, "not found") || strings.Contains(outputLower, "no such file") ||
		...
		return fmt.Errorf("mount failed: %w\nOutput: %s\nHint: Does the share exist? Check with 'dittofsctl share list'", err, output)
	}
	...
	if strings.Contains(outputLower, "authentication") || strings.Contains(outputLower, "login") ||
		...
		return fmt.Errorf("mount failed: %w\nOutput: %s\nHint: Check your credentials with 'dittofsctl login'", err, output)
	}
	...
	// Generic error
	return fmt.Errorf("mount failed: %w\nOutput: %s\nHint: Server may be running on port %d. Check with 'dittofsctl adapter list'", err, output, port)
}
