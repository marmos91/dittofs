...
- [ ] Unified Lock Manager embedded in metadata service
- [ ] Lock state persistence in metadata store (per-share)
- [ ] Flexible lock model (native semantics, translate at boundary)
- [ ] NLM protocol (RPC program 100021) for NFSv3
- [ ] NSM protocol (RPC program 100024) for crash recovery
- [ ] SMB2/3 lease support (Read, Write, Handle leases)
- [ ] Cross-protocol lock coordination (NLM ↔ SMB)
- [ ] Grace period handling for server restarts
- [ ] Per-adapter connection pool (unified stateless/stateful)
- [ ] E2E tests for locking scenarios
...
|----------|-----------|---------|
| NLM before NFSv4 | Build locking foundation first, reuse for NFSv4 | — Pending |
| Unified Lock Manager | Single lock model for NFS+SMB, translate at boundary | — Pending |
| Lock state in metadata store | Atomic with file operations, survives restarts | — Pending |
| Flexible lock model | Preserve native semantics (NLM/NFSv4/SMB), translate at boundary | — Pending |
| Full SMB2/3 leases in v1.0 | Cross-protocol consistency from day one | — Pending |
