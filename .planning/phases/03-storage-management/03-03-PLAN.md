...
Note: This plan depends only on 03-02 (S3 types) since the webhook validates S3 Secret references. Cache configuration from 03-01 is not validated by the webhook.
...
<task type="auto">
  <name>Task 1: Create DittoServerValidator with client injection and validation implementation</name>
  <files>k8s/dittofs-operator/api/v1alpha1/dittoserver_webhook.go</files>
  <action>
Restructure the webhook to use a separate validator type with client access. This task includes BOTH the scaffold AND the validation implementation to ensure the code compiles.
...
5. Add validateDittoServerWithClient method (the actual validation logic):
```go
...
6. Keep the existing DittoServer methods (ValidateCreate, ValidateUpdate, ValidateDelete, validateDittoServer, validatePorts) as they are for the default webhook path.
  </action>
  <verify>
Run:
```bash
...
grep "validateDittoServerWithClient" api/v1alpha1/dittoserver_webhook.go
grep "StorageClass" api/v1alpha1/dittoserver_webhook.go
grep "S3 credentials Secret" api/v1alpha1/dittoserver_webhook.go
```
Build should succeed, DittoServerValidator, SetupDittoServerWebhookWithManager, validateDittoServerWithClient, StorageClass validation, and S3 Secret validation should all exist.
  </verify>
  <done>DittoServerValidator struct exists with Client field, SetupDittoServerWebhookWithManager function exists, validateDittoServerWithClient implements StorageClass and S3 Secret validation</done>
</task>

<task type="auto">
  <name>Task 2: Update main.go to use new webhook setup</name>
...
<task type="auto">
  <name>Task 3: Add webhook validation tests</name>
...
grep "TestStorageClassValidation\|TestS3SecretWarning\|TestS3SecretMissingKeys" api/v1alpha1/dittoserver_webhook_test.go
```
Tests should pass, all three test functions should exist.
  </verify>
