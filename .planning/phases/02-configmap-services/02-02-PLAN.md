...
In the `reconcileStatefulSet` function, BEFORE setting the StatefulSet spec, compute the hash and set annotation:
...
func (r *DittoServerReconciler) reconcileStatefulSet(ctx context.Context, dittoServer *dittoiov1alpha1.DittoServer, replicas int32) error {
    ...
    // Generate config to compute hash
...
3. Compute config hash BEFORE CreateOrUpdate
...
3. Verify the annotation constant is used: `grep -r "dittofs.io/config-hash" k8s/dittofs-operator/`
  ...
  <done>
Controller computes and applies checksum annotation to StatefulSet pod template. Annotation changes when ConfigMap content or referenced Secrets change.
  </done>
</task>
