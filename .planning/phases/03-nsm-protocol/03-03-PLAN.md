...
must_haves:
  truths:
    ...
    - "Lock cleanup processes blocking queue waiters"
...
import (
    ...
    "github.com/marmos91/dittofs/internal/logger"
    nlmXdr "github.com/marmos91/dittofs/internal/protocol/nlm/xdr"
)
...
func DecodeFreeAllRequest(data []byte) (*FreeAllRequest, error) {
    ...
    // FREE_ALL args is just an nlm4_lock with only the caller_name used
    // But simpler implementations just send the name
    // We'll decode a string (the client name)
    name, err := nlmXdr.DecodeString(r)
    if err != nil {
        return nil, err
    }
...
// Releases all locks held by the specified client.
...
func (h *Handler) FreeAll(ctx *NLMHandlerContext) ([]byte, error) {
    ...
    // Get metadata service to release locks
    metaSvc := h.runtime.GetMetadataService()
    if metaSvc == nil {
        logger.Error("FREE_ALL: no metadata service available")
        return EncodeFreeAllResponse()
    }

    // Collect all shares and release locks from each
    totalReleased := 0
    affectedFiles := make(map[string]bool)

    // List all shares and iterate
    shares := h.runtime.ListShares()
    for _, share := range shares {
        // Get lock manager for this share
        lm := metaSvc.GetLockManagerForShare(share.Name)
        if lm == nil {
            continue
        }

        // Get all locks and find those matching our client prefix
        locks := lm.GetAllLocks()
        for _, lock := range locks {
            if strings.HasPrefix(lock.OwnerID, clientIDPrefix) {
                // Release this lock
                if err := lm.Unlock(context.Background(), lock.FileID, lock.OwnerID); err != nil {
                    // Per CONTEXT.md: best effort, log and continue
                    logger.Warn("FREE_ALL: failed to release lock",
                        "file", lock.FileID,
                        "owner", lock.OwnerID,
                        "error", err)
                } else {
                    totalReleased++
                    affectedFiles[lock.FileID] = true
                }
            }
        }
    }
    ...
    // Record metrics
    if h.metrics != nil {
        h.metrics.LocksReleasedByFreeAll.Add(float64(totalReleased))
    }
...
func (h *Handler) processWaitersForFile(ctx context.Context, fileID string) {
    ...
    // Get waiters for this file
    waiters := h.blockingQueue.GetWaitersForFile(fileID)
    if len(waiters) == 0 {
        return
    }
    ...
    // Process each waiter - attempt to acquire their lock
    for _, waiter := range waiters {
        ...
        // This is done asynchronously through the unlock callback mechanism
        // The actual processing happens in the adapter's unlock callback
    }
}
```
...
Note: The NLMHandlerContext needs a Data field to pass request bytes. Check existing context and add if needed.
  </action>
  <verify>
```bash
...
```
  ...
  <done>FREE_ALL handler implemented for bulk lock release with blocking queue waiter processing</done>
</task>
...
<task type="auto">
  <name>Task 5: Integrate Notifier with NFS adapter startup</name>
...
3. Wire up crash handler to FREE_ALL
...
// Create onClientCrash handler that calls FREE_ALL
onClientCrash := func(ctx context.Context, clientID string) error {
    // Build FREE_ALL request and handle it
    // The clientID from NSM is the hostname
    freeAllCtx := &nlmHandlers.NLMHandlerContext{
        Context:    ctx,
        ClientAddr: "",
        Data:       []byte(clientID), // Simplified - actual encoding needed
    }

    // Call FREE_ALL handler directly
    _, err := adapter.nlmHandler.FreeAll(freeAllCtx)
    return err
}
...
The exact integration depends on the adapter's structure. Mirror the pattern used for NLM blocking queue integration.
  </action>
  <verify>
```bash
...
```
  ...
  <done>NSM Notifier integrated with adapter startup - loads registrations and sends SM_NOTIFY on restart</done>
</task>
...
- FREE_ALL NLM handler releases all locks matching client prefix
- FREE_ALL processes blocking queue waiters after lock release
