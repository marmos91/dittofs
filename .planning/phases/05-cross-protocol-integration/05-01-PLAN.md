...
depends_on: []
files_modified:
  - pkg/metadata/lock/unified_view.go
...
must_haves:
  ...
  artifacts:
    - path: "pkg/metadata/lock/unified_view.go"
...
Output: UnifiedLockView struct with query API, NLM holder translation helpers, and Prometheus metrics for cross-protocol events.
</objective>
...
<task type="auto">
  ...
  <files>pkg/metadata/lock/unified_view.go</files>
  <action>
Create new file `pkg/metadata/lock/unified_view.go` with:

1. **FileLocksInfo struct** - Contains query results:
   ```go
   type FileLocksInfo struct {
       ByteRangeLocks []*EnhancedLock // NLM/SMB byte-range locks
       Leases         []*EnhancedLock // SMB2/3 leases (whole-file)
   }
   ```
...
3. **NewUnifiedLockView(store LockStore)** - Constructor

4. **GetAllLocksOnFile(ctx context.Context, fileHandle FileHandle) (*FileLocksInfo, error)**:
...
5. **HasConflictingLocks(ctx context.Context, fileHandle FileHandle, lockType LockType) (bool, []*EnhancedLock, error)**:
...
Add comprehensive doc comments explaining cross-protocol usage.
  ...
  <verify>
    ...
    go test ./pkg/metadata/lock/... -run TestUnified -v
  </verify>
  <done>
    UnifiedLockView can query all locks on a file and separate them by type (byte-range vs lease).
  </done>
</task>
...
<task type="auto">
  ...
  <action>
Create new file `pkg/metadata/lock/cross_protocol.go` with:
...
go doc github.com/marmos91/dittofs/pkg/metadata/lock | grep -E "UnifiedLockView|FileLocksInfo|NLMHolderInfo"
```
</verification>
