...
### Wave 1 — Progress tracker (updated 2026-05-29)

| # | Area | PR-A (audit) | PR-B (fix) | Notes |
|---|---|---|---|---|
...
| 5 | Lock manager + ACL | ✅ **#851 MERGED** `e302663d` | ✅ **DONE** — PR-B1 `#852`+`#856` (persistence) + PR-B2 **`#860`** `cfc2f7d1` (grace/reclaim) + PR-B3 **`#865`** `82d62a3f` (two-store cross-scan). **All 3 HIGH closed.** | 3 HIGH/4 MED/12 LOW; ACL clean (0 HIGH). PR-B1: 4 review rounds/14 bugs/0 stayed-buggy (field-drop + ordering generators sealed). PR-B2: 3 HIGH found post-impl (orphan grace timer, NLM ClientID ephemeral-port, coordinator capture-vs-live) all fixed. PR-B3 closes H-3 (`lm.locks` vs `lm.unifiedLocks` cross-scan) + xproto H1/H2. 4 MED/12 LOW deferred as issues. |
| 6 | Metadata stores + service | 🔄 audit in progress | — | `pkg/metadata/store/{memory,postgres,badger}` + storetest conformance + metadata service/handle/routing layer (lock+acl already done under #5). Recent bug cluster in postgres (#838/#839/#853). |
| 7–11 | Runtime, Snapshot, Config, CLI, Operator | ⏳ queued | — | Per perf-leverage order. |
