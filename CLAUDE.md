...
2. **Every operation carries an `*metadata.AuthContext`.** Created in
   ...
   Tree Connect check (`internal/adapter/smb/handlers/tree_connect.go`). The export
   auth-flavor policy (`AllowAuthSys`, `RequireKerberos`, `MinKerberosLevel`) and the netgroup
   client allowlist live in `CheckExportAccess` (`internal/adapter/nfs/auth/export_access.go`),
   which MOUNT, the NFSv3 dispatch and the NFSv4 auth-context builder all call. The flavor
   gates run per operation, not only at mount; the netgroup allowlist is applied by MOUNT and
   by the v4 builder, and the v3 per-operation gate passes no netgroup lookup.
