...
// These tests verify creation, listing, editing, and deletion of block stores,
...
func TestBlockStoresCRUD(t *testing.T) {
	...
	t.Run("create memory store", func(t *testing.T) {
		...
		t.Cleanup(func() {
			_ = cli.DeleteBlockStore(storeName)
		})

		store, err := cli.CreateBlockStore(storeName, "memory")
	...
	// Create S3 block store (without actual S3)
	t.Run("create s3 store", func(t *testing.T) {
		...
		// An S3 store requires bucket + credentials; supply dummy keys so
		...
		store, err := cli.CreateBlockStore(storeName, "s3",
	...
	t.Run("list stores", func(t *testing.T) {
		...
		t.Cleanup(func() {
			_ = cli.DeleteBlockStore(store1Name)
			_ = cli.DeleteBlockStore(store2Name)
		})

		// Create two stores
		_, err := cli.CreateBlockStore(store1Name, "memory")
		require.NoError(t, err, "Should create first store")

		_, err = cli.CreateBlockStore(store2Name, "memory")
		require.NoError(t, err, "Should create second store")

		// List all stores
		stores, err := cli.ListBlockStores()
	...
	t.Run("cannot delete store in use", func(t *testing.T) {
		...
		err = cli.DeleteBlockStore(blockStoreName)
	...
	t.Run("get store by name", func(t *testing.T) {
		...
		// Create store
		created, err := cli.CreateBlockStore(storeName, "memory")
		require.NoError(t, err, "Should create store")

		// Get store by name
		fetched, err := cli.GetBlockStore(storeName)
	...
	// Share creation requires an existing block store
	t.Run("error/share requires block store", func(t *testing.T) {
		metaStoreName := helpers.UniqueTestName("meta_noblk")
		t.Cleanup(func() {
			_ = cli.DeleteMetadataStore(metaStoreName)
		})
		...
		_, err = cli.CreateShare("/"+helpers.UniqueTestName("share_noblk"), metaStoreName, "nonexistent-block")
		require.Error(t, err, "Should fail to create share with missing block store")
	})
}
