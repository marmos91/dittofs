...
| smb2.notify.rec | rmdir refused after a rename out of the directory | `smb2_util_rmdir` on the emptied source directory is refused with `NT_STATUS_DIRECTORY_NOT_EMPTY`; `notify.c:634` requires `NT_STATUS_OK`. The rename at `notify.c:609` that this row previously recorded now returns `NT_STATUS_OK` — the destination-parent sharing gate was corrected in #2135 — so the test reaches this later assertion for the first time. Fails identically on both profiles. | [#2140](https://github.com/marmos91/dittofs/issues/2140) |
...
| smb2.notify.mask-change | rmdir refused after a rename out of the directory (was: Samba notify-mask quirk) | **The recorded rationale below is still unreached.** The test now fails at `notify.c:829`, on `smb2_util_rmdir` of the emptied source directory returning `NT_STATUS_DIRECTORY_NOT_EMPTY` — the same defect as `smb2.notify.rec`, tracked as [#2140](https://github.com/marmos91/dittofs/issues/2140). It previously failed one step earlier, at `notify.c:813`, on a rename refused with `STATUS_SHARING_VIOLATION`; correcting that gate in #2135 moved the failure later rather than clearing it. The mask-stickiness rationale below has now been deferred twice and has never been observed either way, so it should be re-tested once [#2140](https://github.com/marmos91/dittofs/issues/2140) lands rather than trusted — including this row's placement in this section, which assumes the test is unimplementable rather than blocked. Original rationale: asserts that, once a CHANGE_NOTIFY completion-filter mask has been armed on a directory handle, re-issuing CHANGE_NOTIFY with a different mask MUST observe the original mask only until the handle is closed (test source: `source4/torture/smb2/notify.c:771-772` — "Now try and change the mask to include other events. This should not work - once the mask is set on a directory h1 it seems to be fixed until the fnum is closed"). MS-SMB2 §3.3.5.19 does not specify mask-coalescing across separate CHANGE_NOTIFY requests on the same handle, and the test has a long-standing "never passed individually" history against DittoFS independent of test order. Surrounding scenarios (cross-tree dir/file rename plumbing, recursion-flag-mixed reqs on the same FID) are also Samba implementation conventions. |
