...
| smb2.notify.rec | CHANGE_NOTIFY with an empty completion filter is refused | `smb2_notify_send` at `notify.c:627` arms a recursive CHANGE_NOTIFY with `completion_filter = 0`; its `smb2_notify_recv` at `notify.c:641` answers `NT_STATUS_INVALID_PARAMETER` where `notify.c:642` requires `NT_STATUS_OK`. The test relies on the filter armed by the earlier request on the same handle still applying — the same mask-stickiness behaviour `smb2.notify.mask-change` asserts, which MS-SMB2 §3.3.5.19 does not specify. The rmdir refusal this row previously recorded at `notify.c:634` is fixed: both rmdirs and the unlink at `notify.c:631`/`:634`/`:636` now return `NT_STATUS_OK`. Samba also carries this test as a known failure (`^samba3.smb2.notify.rec` in `selftest/knownfail`). | [#2164](https://github.com/marmos91/dittofs/issues/2164) |
...
| smb2.notify.mask-change | Samba notify-mask stickiness quirk | **The rationale below has now been reached and observed for the first time.** The test fails at `notify.c:834`, on the `smb2_notify_recv` at `:833` answering `NT_STATUS_CANCELLED` where `NT_STATUS_OK` is required — the re-issued CHANGE_NOTIFY did not observe the mask armed by the earlier request on the same handle. The rmdir refusal this row previously recorded at `notify.c:829` is fixed: both rmdirs and the unlink at `notify.c:827`/`:829`/`:831` now return `NT_STATUS_OK`. That was the second deferral of this rationale, and the test now sits on the assertion the rationale describes, so this row's placement here is supported by observation rather than assumed. Rationale: asserts that, once a CHANGE_NOTIFY completion-filter mask has been armed on a directory handle, re-issuing CHANGE_NOTIFY with a different mask MUST observe the original mask until the handle is closed (test source: `source4/torture/smb2/notify.c:771-772` — "Now try and change the mask to include other events. This should not work - once the mask is set on a directory h1 it seems to be fixed until the fnum is closed"). MS-SMB2 §3.3.5.19 does not specify mask-coalescing across separate CHANGE_NOTIFY requests on the same handle, and the test has a long-standing "never passed individually" history against DittoFS independent of test order. Surrounding scenarios (cross-tree dir/file rename plumbing, recursion-flag-mixed reqs on the same FID) are also Samba implementation conventions. **Skipped by `run.sh` per-subtest split.** Once a directory handle could hold several outstanding CHANGE_NOTIFY requests, the two this test issues both stay pending — correctly, since the sticky mask matches none of the events it generates — and it blocks in `smb2_notify_recv` until the suite budget expires, leaving every later subtest UNGRADED. It previously ended in milliseconds only because the second request evicted the first with `STATUS_CANCELLED`, which was itself the defect. Same workaround shape as `scan.scan` and `dirlease.oplocks`. |
