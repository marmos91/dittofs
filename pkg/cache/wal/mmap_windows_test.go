...
func TestMmapPersister_ConcurrentAppends(t *testing.T) {
	dir := t.TempDir()
	...
	for g := 0; g < numGoroutines; g++ {
		...
		go func(goroutineID int) {
			...
			for i := 0; i < writesPerGoroutine; i++ {
				entry := &BlockWriteEntry{
					PayloadID:     fmt.Sprintf("file-g%d", goroutineID),
					ChunkIdx:      0,
					BlockIdx:      uint32(i),
					OffsetInBlock: 0,
					Data:          []byte(fmt.Sprintf("data-g%d-i%d", goroutineID, i)),
				}
				if err := p.AppendBlockWrite(entry); err != nil {
					errCh <- fmt.Errorf("goroutine %d, write %d: %w", goroutineID, i, err)
					return
				}
			}
		}(g)
	...
	if len(recovered.Entries) != expectedEntries {
		t.Fatalf("Recover() got %d entries, want %d", len(recovered.Entries), expectedEntries)
	}

	// Verify each goroutine's entries are present
	entryCounts := make(map[string]int)
	for _, entry := range recovered.Entries {
		entryCounts[entry.PayloadID]++
	}

	for g := 0; g < numGoroutines; g++ {
		key := fmt.Sprintf("file-g%d", g)
		if entryCounts[key] != writesPerGoroutine {
			t.Errorf("goroutine %d: got %d entries, want %d", g, entryCounts[key], writesPerGoroutine)
		}
	}
}
