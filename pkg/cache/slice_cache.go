...
//
// SliceCache provides a slice-aware caching layer for the Chunk/Slice/Block
...
var (
	// ErrSliceCacheClosed is returned when operations are attempted on a closed cache.
	ErrSliceCacheClosed = errors.New("slice cache is closed")
...
// SliceCacheStats provides cache statistics for monitoring.
type SliceCacheStats struct {
	// TotalSize is the total bytes currently in the cache.
	TotalSize uint64

	// DirtySize is the bytes that haven't been flushed yet.
	DirtySize uint64

	// PendingSlices is the count of unflushed slices.
	PendingSlices int

	// FlushedSlices is the count of flushed slices still in cache.
	FlushedSlices int

	// FileCount is the number of files with cached data.
	FileCount int

	// ChunkCount is the total number of cached chunks.
	ChunkCount int

	// MaxSize is the configured maximum cache size.
	MaxSize uint64

	// Hits is the number of cache read hits.
	Hits uint64

	// Misses is the number of cache read misses.
	Misses uint64
}

// HitRate returns the cache hit rate as a float64 (0.0 to 1.0).
func (s SliceCacheStats) HitRate() float64 {
...
// SliceCache is the mandatory cache layer for all content operations.
//
...
type SliceCache interface {
	...
	//   - error: ErrFileNotInCache, ErrSliceCacheClosed, or context error
	...
	//   - error: ErrFileNotInCache if no cached data, ErrSliceCacheClosed if closed
	...
	//   - error: ErrSliceCacheClosed if closed
	...
	//   - All operations return ErrSliceCacheClosed
	//
