...
type RestoreOrphan struct {
	...
	// suffix embedded in every generated temp schema name (see).
...
// prefix. Used by restore's startup orphan sweep to find
...
// []RestoreOrphan{} rather than nil so sweep can range safely.
//
...
//	 temp schemas encode a millisecond-precision timestamp in the
...
// permission denials, query timeouts). ListSchemasByPrefix is REQUIRED (non-optional)
// per silent degradation is unacceptable.
...
// pool. Used by restore's CommitSwap path to reclaim the old
// schema's storage after a successful swap, and by the orphan sweep
...
// operator-controlled strings temp schemas are generated from
