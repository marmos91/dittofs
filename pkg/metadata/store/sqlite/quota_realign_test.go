...
// without the guard, over-counting by exactly the raced transaction. This copy
// needs no database server; the postgres one pins the backend whose connection
// pool lets many writers reach the gap at once.
func TestRecomputeUsageCannotSplitACommitFromItsFold(t *testing.T) {
	...
	restore := sqlite.SetAfterCommitFold(func() {
