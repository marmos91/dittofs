...
// LockConfig contains configuration settings for the lock manager.
//
...
// LockLimits tracks current lock usage for limit enforcement.
//
// Thread Safety:
// LockLimits is safe for concurrent use by multiple goroutines.
...
// NewLockLimits creates a new LockLimits tracker.
...
func (ll *LockLimits) CheckLimits(config LockConfig, fileHandle, clientID string) error {
	ll.mu.RLock()
	defer ll.mu.RUnlock()
...
func (ll *LockLimits) IncrementCounts(fileHandle, clientID string) {
	ll.mu.Lock()
	defer ll.mu.Unlock()
...
func (ll *LockLimits) DecrementCounts(fileHandle, clientID string) {
	ll.mu.Lock()
	defer ll.mu.Unlock()
...
func (ll *LockLimits) DecrementCountsN(fileHandle, clientID string, count int) {
	if count <= 0 {
		return
	}
...
// LockStats contains current lock usage statistics.
...
func (ll *LockLimits) GetStats() LockStats {
	ll.mu.RLock()
	defer ll.mu.RUnlock()
...
func (ll *LockLimits) GetFileCount(fileHandle string) int {
	ll.mu.RLock()
	defer ll.mu.RUnlock()
...
func (ll *LockLimits) GetClientCount(clientID string) int {
	ll.mu.RLock()
	defer ll.mu.RUnlock()
...
func (ll *LockLimits) GetTotalCount() int {
	ll.mu.RLock()
	defer ll.mu.RUnlock()
