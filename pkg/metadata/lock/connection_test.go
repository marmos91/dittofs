...
func TestUnregisterClient_ImmediateRelease_TTL0(t *testing.T) {
	...
	ct.RegisterClient("client1", "nfs", "192.168.1.1", 0)
	ct.UnregisterClient("client1")

	// Give callback time to execute (it runs in goroutine)
	time.Sleep(50 * time.Millisecond)

	if disconnectedClient != "client1" {
		t.Errorf("Expected disconnect callback for client1, got %q", disconnectedClient)
	}
...
func TestUnregisterClient_DeferredRelease_TTL_Positive(t *testing.T) {
	...
	ct.RegisterClient("client1", "nfs", "192.168.1.1", 100*time.Millisecond)
	ct.UnregisterClient("client1")
	...
	if disconnectedClient != "client1" {
		t.Errorf("Expected disconnect callback after TTL, got %q", disconnectedClient)
	}
}
...
func TestListClients_FilterByAdapter(t *testing.T) {
	...
	ct.RegisterClient("nfs1", "nfs", "192.168.1.1", 0)
	ct.RegisterClient("nfs2", "nfs", "192.168.1.2", 0)
	ct.RegisterClient("smb1", "smb", "192.168.1.3", 0)
...
func TestConnectionTracker_Close(t *testing.T) {
	...
	ct.RegisterClient("client1", "nfs", "192.168.1.1", 1*time.Second)
	ct.UnregisterClient("client1")
...
func TestConnectionTracker_ThreadSafety(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(n int) {
			...
			ct.RegisterClient(clientID, "nfs", "192.168.1.1", 0)
...
func TestGetPendingDisconnectCount(t *testing.T) {
	...
	ct.RegisterClient("client1", "nfs", "192.168.1.1", 1*time.Second)
	ct.RegisterClient("client2", "nfs", "192.168.1.2", 1*time.Second)
	ct.UnregisterClient("client1")
