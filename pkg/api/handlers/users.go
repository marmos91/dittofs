...
func (h *UserHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	// Hash password
	passwordHash, err := identity.HashPassword(req.Password)
	if err != nil {
	...
	user.SetNTHashFromPassword(req.Password)
...
func (h *UserHandler) Get(w http.ResponseWriter, r *http.Request) {
	...
	user, err := h.identityStore.GetUser(username)
	if err != nil {
...
func (h *UserHandler) ResetPassword(w http.ResponseWriter, r *http.Request) {
	...
	// Hash new password
	passwordHash, err := identity.HashPassword(req.NewPassword)
	if err != nil {
		InternalServerError(w, "Failed to hash password")
		return
	}
...
func (h *UserHandler) ChangeOwnPassword(w http.ResponseWriter, r *http.Request) {
	...
	// Get current user
	user, err := h.identityStore.GetUser(claims.Username)
	if err != nil {
		if err == identity.ErrUserNotFound {
