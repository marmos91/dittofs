...
func makeHash(b byte) block.ContentHash {
...
func TestDedupLRU_EvictsLRUWhenOverCapacity(t *testing.T) {
	...
	for i, h := range []block.ContentHash{h2, h3, h4} {
		if !lru.Hit(h, "p") {
...
func TestDedupLRU_ConcurrentAccess_NoRace(t *testing.T) {
	lru := newDedupLRU(64)
	keys := make([]block.ContentHash, 32)
	for i := range keys {
...
func TestDedupLRU_ZeroSize_DegradesToNoop(t *testing.T) {
	...
	lru.PutMany([]block.ContentHash{makeHash(2)}, "p")
...
func TestDedupLRU_PutMany_BatchPopulates(t *testing.T) {
	lru := newDedupLRU(8)
	hashes := []block.ContentHash{makeHash(1), makeHash(2), makeHash(3)}
