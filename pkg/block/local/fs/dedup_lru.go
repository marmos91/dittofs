...
func (c *dedupLRU) Hit(hash block.ContentHash, payloadID string) bool {
...
func (c *dedupLRU) Has(hash block.ContentHash, payloadID string) bool {
	if c == nil || c.maxSize <= 0 {
		return false
	}
	c.mu.Lock()
...
func (c *dedupLRU) Put(hash block.ContentHash, payloadID string) {
	if c == nil || c.maxSize <= 0 {
		return
	}
	c.mu.Lock()
...
func (c *dedupLRU) PutMany(hashes []block.ContentHash, payloadID string) {
