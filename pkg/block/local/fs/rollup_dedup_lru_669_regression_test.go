...
func TestRollup_DedupLRU_NotPopulatedBeforePersister(t *testing.T) {
	...
	bc.SetObjectIDPersister(func(_ context.Context, _ string, _ []block.BlockRef, _ block.ObjectID) error {
...
func TestRollup_DedupLRU_ConcurrentRollups_NoErrUnknownHash(t *testing.T) {
	...
	wrapped.addRefOverride = func(ctx context.Context, h block.ContentHash, pid string, ref block.BlockRef) error {
		err := wrapped.inner.AddRef(ctx, h, pid, ref)
		if errors.Is(err, block.ErrUnknownHash) {
