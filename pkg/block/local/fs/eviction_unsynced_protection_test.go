...
func TestLRU_UnsyncedChunk_lruEvictOneReturnsEmpty(t *testing.T) {
	...
	for _, h := range []block.ContentHash{hA, hB, hC} {
...
func (s *slowSyncedHashStore) IsSynced(ctx context.Context, h block.ContentHash) (bool, error) {
	time.Sleep(s.delay)
...
func TestLRU_EvictTouchRace(t *testing.T) {
	...
	seen := make(map[block.ContentHash]int, bc.lruList.Len())
