...
type programmableFBS struct {
	...
	// error scenarios.
	addRefOverride func(ctx context.Context, h block.ContentHash, payloadID string, ref block.BlockRef) error
}

func newProgrammableFBS(inner block.EngineFileBlockStore) *programmableFBS {
	return &programmableFBS{inner: inner}
}

func (p *programmableFBS) GetByHash(ctx context.Context, h block.ContentHash) (*block.FileBlock, error) {
	return p.inner.GetByHash(ctx, h)
}
func (p *programmableFBS) Put(ctx context.Context, b *block.FileBlock) error {
...
func (p *programmableFBS) AddRef(ctx context.Context, h block.ContentHash, payloadID string, ref block.BlockRef) error {
	p.addRefCalls.Add(1)
...
func hashOfSingleChunk(payload []byte) block.ContentHash {
...
func TestRollup_LRUHit_SkipsStoreChunk(t *testing.T) {
	...
	if err := mem.Put(ctx, &block.FileBlock{
...
func TestRollup_AddRefReturnsErrUnknownHash_FallsBackToStoreChunk(t *testing.T) {
	...
	bc.dedupLRU.Put(h, pid)
	wrapped.addRefOverride = func(_ context.Context, _ block.ContentHash, _ string, _ block.BlockRef) error {
...
func TestRollup_LRUHit_NoBlockStateMutation(t *testing.T) {
	...
	const seedState = block.BlockStateRemote
	if err := mem.Put(ctx, &block.FileBlock{
		ID:       seedID,
		Hash:     h,
		State:    seedState,
