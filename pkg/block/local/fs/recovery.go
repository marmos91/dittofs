...
func (bc *FSStore) Recover(ctx context.Context) error {
	...
	err := filepath.WalkDir(bc.baseDir, func(path string, d os.DirEntry, err error) error {
		...
		if fb.BlockStoreKey != "" && fb.State == block.BlockStatePending {
			fb.State = block.BlockStateRemote
			needsUpdate = true
		}
		...
		payloadID, blockIdx, parseErr := block.ParseBlockID(blockID)
		if parseErr == nil {
			end := (blockIdx + 1) * block.BlockSize
			if fb.DataSize > 0 && fb.DataSize < uint32(block.BlockSize) {
				end = blockIdx*block.BlockSize + uint64(fb.DataSize)
			}
