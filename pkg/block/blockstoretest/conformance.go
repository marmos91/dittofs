...
type Factory func(t *testing.T) (block.Store, func())
...
//   - Walk callback may return block.ErrStopWalk to exit cleanly
...
func testDelete(t *testing.T, factory Factory) {
	...
	if _, err := bs.Get(ctx, h); !errors.Is(err, block.ErrChunkNotFound) {
...
func testWalk(t *testing.T, factory Factory) {
	...
	want := make(map[block.ContentHash]int64, len(payloads))
	...
	seen := make(map[block.ContentHash]block.Meta, len(payloads))
	err := bs.Walk(ctx, func(h block.ContentHash, m block.Meta) error {
		seen[h] = m
		return nil
	})
	if err != nil {
...
// block.ErrInvalidOffset and a non-positive length MUST surface
// block.ErrInvalidSize. Unlike the offset-past-EOF case (which the
...
func testGetRangeInvalidBounds(t *testing.T, factory Factory) {
	...
	if _, err := bs.GetRange(ctx, h, -1, 4); !errors.Is(err, block.ErrInvalidOffset) {
		t.Fatalf("GetRange offset=-1: want ErrInvalidOffset, got %v", err)
	}

	// Zero and negative length are ErrInvalidSize.
	if _, err := bs.GetRange(ctx, h, 0, 0); !errors.Is(err, block.ErrInvalidSize) {
		t.Fatalf("GetRange length=0: want ErrInvalidSize, got %v", err)
	}
	if _, err := bs.GetRange(ctx, h, 0, -4); !errors.Is(err, block.ErrInvalidSize) {
...
func testConcurrentPutWalkNoDuplicates(t *testing.T, factory Factory) {
	...
	hashes := make([]block.ContentHash, writers)
	for i := 0; i < writers; i++ {
	...
	go func() {
		defer wg.Done()
		err := bs.Walk(ctx, func(h block.ContentHash, _ block.Meta) error {
			walkMu.Lock()
			seen[h]++
			walkMu.Unlock()
			return nil
