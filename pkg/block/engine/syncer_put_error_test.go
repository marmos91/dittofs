...
func (f *failingPutRemote) Put(_ context.Context, _ block.ContentHash, _ []byte) error {
	return errBoomPut
}
...
func newOneHashLocalStore(t *testing.T, hash block.ContentHash, data []byte) *oneHashLocalStore {
	t.Helper()
	ms := memorylocal.New()
...
func (o *oneHashLocalStore) ListUnsynced(_ context.Context) iter.Seq2[block.ContentHash, error] {
...
func TestMirrorLoop_PropagatesPutError(t *testing.T) {
	...
	hash := block.ContentHash(blake3.Sum256(payload))
