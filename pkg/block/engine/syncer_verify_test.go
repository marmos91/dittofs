...
func (r *countingPutRemote) Put(_ context.Context, _ block.ContentHash, _ []byte) error {
	r.puts.Add(1)
	return nil
}
...
func TestMirrorOnce_DetectsLocalCorruption_RefusesUpload(t *testing.T) {
	...
	claimedHash := block.ContentHash{0xDE, 0xAD, 0xBE, 0xEF}
	...
	if block.ContentHash(blake3.Sum256(corruptPayload)) == claimedHash {
