...
type cacheInterface interface {
	Get(hash block.ContentHash) ([]byte, bool)
	Put(hash block.ContentHash, data []byte)
	OnRead(payloadID string, hashes []block.ContentHash, fileSize uint64)
	InvalidateFile(payloadID string, removedHashes []block.ContentHash)
	Stats() CacheStats
	Close() error
}
...
type Cache struct {
	mu       sync.RWMutex
	entries  map[block.ContentHash]*list.Element // hash -> element holding *cacheEntry
...
func (nullCache) Get(block.ContentHash) ([]byte, bool)       { return nil, false }
func (nullCache) Put(block.ContentHash, []byte)              {}
func (nullCache) OnRead(string, []block.ContentHash, uint64) {}
func (nullCache) InvalidateFile(string, []block.ContentHash) {}
...
func newCacheNoWorkers(maxBytes int64) *Cache {
	...
	return &Cache{
		entries:  make(map[block.ContentHash]*list.Element),
		lru:      list.New(),
...
func (c *Cache) Get(hash block.ContentHash) ([]byte, bool) {
...
func (c *Cache) Put(hash block.ContentHash, data []byte) {
	if c == nil || c.closed.Load() {
		return
	}
...
func (c *Cache) OnRead(payloadID string, hashes []block.ContentHash, fileSize uint64) {
	if c == nil || c.closed.Load() {
		return
	}
	if len(hashes) == 0 {
	...
	if !ok {
		tr = &seqTracker{
			lastHashes: make([]block.ContentHash, 0, seqThreshold),
			depth:      1,
		}
	...
	var toPrefetch []block.ContentHash
	if len(tr.lastHashes) >= seqThreshold {
		want := tr.depth
...
func (c *Cache) Close() error {
	...
	c.mu.Lock()
	c.entries = make(map[block.ContentHash]*list.Element)
	c.lru.Init()
	c.curBytes = 0
	c.mu.Unlock()
