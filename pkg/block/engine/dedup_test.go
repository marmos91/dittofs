...
func makeSpeculativeBlocks(tags ...byte) []block.BlockRef {
	out := make([]block.BlockRef, len(tags))
	for i, tag := range tags {
		var h block.ContentHash
		h[0] = tag
		out[i] = block.BlockRef{
			Hash:   h,
...
// Pending".
func pendingStates(n int) []block.BlockState {
	out := make([]block.BlockState, n)
	for i := range out {
		out[i] = block.BlockStatePending
	}
	return out
}
...
func TestDedup_TriggerCondition(t *testing.T) {
	...
	t.Run("EmptyBlocks_NoFindCall", func(t *testing.T) {
		...
		hit, err := m.trySpeculativeFileLevelDedup(ctx, "pid", nil, block.ObjectID{}, nil)
		if err != nil {
			t.Fatalf("err: %v", err)
		}
		if hit {
	...
	t.Run("OneBlockNotPending_NoFindCall", func(t *testing.T) {
		...
		states[1] = block.BlockStateRemote // one already Remote
		hit, err := m.trySpeculativeFileLevelDedup(ctx, "pid", blocks, block.ObjectID{}, states)
		if err != nil {
			t.Fatalf("err: %v", err)
		}
		if hit {
	...
	t.Run("AllConditionsMet_FindCalledOnce", func(t *testing.T) {
		...
		_, err := m.trySpeculativeFileLevelDedup(ctx, "pid", blocks, block.ObjectID{}, states)
		if err != nil {
			t.Fatalf("err: %v", err)
		}
		...
		if len(fc.findCalls) == 1 {
			want := block.ComputeObjectID(blocks)
...
func TestDedup_ShortCircuit_HitFlow(t *testing.T) {
	...
	hit, err := m.trySpeculativeFileLevelDedup(ctx, "pid",
		speculative, block.ObjectID{}, pendingStates(len(speculative)))
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if !hit {
...
// block.ErrConflict), the test still compiles — it just relies on
