...
func hashN(b byte) block.ContentHash {
...
func TestCache_InvalidateFile_Surgical(t *testing.T) {
	...
	c.InvalidateFile("payloadA", []block.ContentHash{hA1, hA2, hA3})

	// hA1, hA2, hA3 must be gone.
	for _, h := range []block.ContentHash{hA1, hA2, hA3} {
		_, ok := c.Get(h)
		assert.Falsef(t, ok, "%x should be invalidated", h[31])
	}

	// hA4, hA5 must remain.
	for _, h := range []block.ContentHash{hA4, hA5} {
		_, ok := c.Get(h)
		assert.Truef(t, ok, "A's untouched hash %x should remain", h[31])
	}

	// B's hashes must be untouched.
	for _, h := range []block.ContentHash{hB1, hB2} {
		_, ok := c.Get(h)
...
type recordingLoader struct {
	mu        sync.Mutex
	calls     []block.ContentHash
	inFlight  atomic.Int32
	maxInFly  atomic.Int32
	wait      time.Duration // synthetic latency to expose concurrency
	dataMaker func(block.ContentHash) []byte
}

func newRecordingLoader(latency time.Duration) *recordingLoader {
	return &recordingLoader{
		wait: latency,
		dataMaker: func(h block.ContentHash) []byte {
			return []byte{h[31]}
		},
	}
}

func (r *recordingLoader) load(_ context.Context, h block.ContentHash) ([]byte, error) {
...
func TestCache_SequentialDetection_CACHE03(t *testing.T) {
	...
	hashes := []block.ContentHash{hashN(0), hashN(1), hashN(2), hashN(3), hashN(4)}

	// Each OnRead simulates a single block-read for a payloadID.
	for i := 0; i < 3; i++ {
		c.OnRead("file1", []block.ContentHash{hashes[i]}, 5*1024)
	}
	...
	c.OnRead("file1", []block.ContentHash{hashes[3]}, 5*1024)
...
func TestCache_OnRead_NonSequentialResetsTracker(t *testing.T) {
	...
	c.OnRead("file1", []block.ContentHash{hashN(1)}, 1024)
	c.OnRead("file1", []block.ContentHash{hashN(2)}, 1024)
	...
	prev := loader.callCount()
	c.OnRead("file1", []block.ContentHash{}, 1024) // reset signal — empty triggers nothing
	c.OnRead("file1", []block.ContentHash{hashN(10)}, 1024)
	c.OnRead("file1", []block.ContentHash{hashN(11)}, 1024)
	...
	c.OnRead("file1", []block.ContentHash{hashN(12)}, 1024)
	if !loader.waitForCalls(prev+1, 500*time.Millisecond) {
...
func TestCache_BoundedConcurrency(t *testing.T) {
	...
	for f := 0; f < 25; f++ {
		...
		c.OnRead(pid, []block.ContentHash{hashN(byte(f*4 + 0))}, 1024)
		c.OnRead(pid, []block.ContentHash{hashN(byte(f*4 + 1))}, 1024)
		c.OnRead(pid, []block.ContentHash{hashN(byte(f*4 + 2))}, 1024)
		c.OnRead(pid, []block.ContentHash{hashN(byte(f*4 + 3))}, 1024)
	}
...
func TestCache_Close_StopsWorkers(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		c.OnRead("f", []block.ContentHash{hashN(byte(i))}, 1024)
	}
	...
	prev := loader.callCount()
	c.OnRead("f", []block.ContentHash{hashN(99)}, 1024)
