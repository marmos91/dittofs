...
func newWalkTestStore(t *testing.T) (metadata.MetadataStore, string, metadata.FileHandle) {
	t.Helper()
	ctx := t.Context()
...
func addFile(t *testing.T, store metadata.MetadataStore, shareName string, parent metadata.FileHandle, name, path string) metadata.FileHandle {
	t.Helper()
	ctx := t.Context()
...
func addDir(t *testing.T, store metadata.MetadataStore, shareName string, parent metadata.FileHandle, name, path string) metadata.FileHandle {
	t.Helper()
	ctx := t.Context()
	handle, err := store.GenerateHandle(ctx, shareName, path)
	if err != nil {
