...
// fakeBlockStore is an in-memory blockstore.BlockStore for the migration
// tests. Records the chunks written so post-run assertions can verify
// what was Put.
type fakeBlockStore struct {
	mu       sync.Mutex
	chunks   map[blockstore.ContentHash][]byte
	putCount int64
	getCount int64
	hasCount int64
}
...
func newFakeBlockStore() *fakeBlockStore {
	return &fakeBlockStore{
		chunks: make(map[blockstore.ContentHash][]byte),
	}
}

func (b *fakeBlockStore) Put(_ context.Context, hash blockstore.ContentHash, data []byte) error {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *fakeBlockStore) Get(_ context.Context, hash blockstore.ContentHash) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *fakeBlockStore) GetRange(_ context.Context, hash blockstore.ContentHash, offset, length int64) ([]byte, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	data, ok := b.chunks[hash]
	if !ok {
...
func (b *fakeBlockStore) Has(_ context.Context, hash blockstore.ContentHash) (bool, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *fakeBlockStore) Delete(_ context.Context, hash blockstore.ContentHash) error {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *fakeBlockStore) Head(_ context.Context, hash blockstore.ContentHash) (blockstore.Meta, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	data, ok := b.chunks[hash]
	if !ok {
	...
	return blockstore.Meta{Size: int64(len(data)), LastModified: time.Now()}, nil
}

func (b *fakeBlockStore) Walk(_ context.Context, fn func(blockstore.ContentHash, blockstore.Meta) error) error {
	b.mu.Lock()
	snap := make(map[blockstore.ContentHash][]byte, len(b.chunks))
	for k, v := range b.chunks {
		snap[k] = v
	}
	b.mu.Unlock()
	for h, data := range snap {
		if err := fn(h, blockstore.Meta{Size: int64(len(data)), LastModified: time.Now()}); err != nil {
...
func (c *corruptingBlockStore) Get(_ context.Context, hash blockstore.ContentHash) ([]byte, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (s *stubMetadataAdapter) UpdateFileBlocks(_ context.Context, handle metadata.FileHandle, blocks []blockstore.BlockRef) error {
	s.mu.Lock()
	defer s.mu.Unlock()
