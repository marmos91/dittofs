...
func hashFor(payload []byte) blockstore.ContentHash {
...
func seedChunks(t *testing.T, bc *FSStore, n int) []blockstore.ContentHash {
	t.Helper()
	ctx := context.Background()
	hashes := make([]blockstore.ContentHash, 0, n)
	for i := 0; i < n; i++ {
...
func collectIter(it func(yield func(blockstore.ContentHash, error) bool)) ([]blockstore.ContentHash, []error) {
	var hashes []blockstore.ContentHash
	var errs []error
	for h, err := range it {
		hashes = append(hashes, h)
...
func sortHashes(in []blockstore.ContentHash) []blockstore.ContentHash {
	out := make([]blockstore.ContentHash, len(in))
	copy(out, in)
...
func hashSetEqual(a, b []blockstore.ContentHash) bool {
	if len(a) != len(b) {
		return false
	}
	as := sortHashes(a)
...
func TestFSStore_ListUnsynced(t *testing.T) {
	...
	t.Run("Partial", func(t *testing.T) {
		...
		synced := []blockstore.ContentHash{hashes[1], hashes[3]}
		for _, h := range synced {
			if err := shs.MarkSynced(ctx, h); err != nil {
				t.Fatalf("MarkSynced: %v", err)
			}
		}
		want := []blockstore.ContentHash{hashes[0], hashes[2], hashes[4]}
