...
func mkBlocks(offsetsAndSizes ...uint64) []blockstore.BlockRef {
	const defaultSize = 4 << 20
	if len(offsetsAndSizes)%2 != 0 {
		// caller passed only offsets — pair them with default size.
		out := make([]blockstore.BlockRef, len(offsetsAndSizes))
		for i, off := range offsetsAndSizes {
			out[i] = blockstore.BlockRef{Offset: off, Size: defaultSize}
		}
		return out
	}
	out := make([]blockstore.BlockRef, len(offsetsAndSizes)/2)
	for i := 0; i < len(offsetsAndSizes); i += 2 {
		out[i/2] = blockstore.BlockRef{Offset: offsetsAndSizes[i], Size: uint32(offsetsAndSizes[i+1])}
	}
	return out
}
...
func TestFindBlocksForRange_Property(t *testing.T) {
	...
	for trial := 0; trial < trials; trial++ {
		...
		for i := range blocks {
			...
			blocks[i] = blockstore.BlockRef{Offset: cursor + gap, Size: sz}
