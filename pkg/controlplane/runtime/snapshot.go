...
func (r *Runtime) runSnapshotOrchestration(
	...
	opStart := time.Now()
	defer func() {
		// Inline snapshot metrics: this defer is the single terminal point for
		// the async create pipeline (terminalErr nil = ready, non-nil =
		// failed). The held-snapshot count and last-success timestamp are
		// already surfaced per-share + restart-safe by the read-through
		// collector, so only the event-driven op count + duration are recorded
		// here.
		result := "ok"
		if terminalErr != nil {
			result = "error"
		}
		r.metrics.RecordSnapshotOp("create", result, time.Since(opStart))
...
func (r *Runtime) RestoreSnapshot(
	...
	// Inline snapshot metrics: time the operator-initiated restore. The
	// startup-recovery rollback path calls restoreSnapshot directly, so it is
	// (correctly) not counted here as an operator restore.
	opStart := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
		...
		r.metrics.RecordSnapshotOp("restore", result, time.Since(opStart))
	}()
...
func (r *Runtime) restoreSnapshot(
	...
	// Re-arm cold reads: ResetLocalState wiped the local tier, so the restored
	// manifest's extents have no journal interval and a read would zero-fill. Seed
	// them cold from the restored FileChunk manifest so reads fault in from remote.
	...
	if remoteVerify {
		...
		logger.Info("snapshot restore: cold intervals seeded from restored manifest",
			"snapshot_id", snapID, "share", shareName)
	}
...
func (r *Runtime) DeleteSnapshot(ctx context.Context, share, snapID string) (err error) {
	// Inline snapshot metrics: record op + duration on every exit, and refresh
	// the held-snapshot count on success.
	opStart := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
		...
		r.metrics.RecordSnapshotOp("delete", result, time.Since(opStart))
	}()
