...
// SnapshotStore provides snapshot CRUD and state-machine operations.
//
// Snapshots are per-share, point-in-time records identified by a UUID. The
// store enforces "at most one in-flight (state='creating') snapshot per share"
// via the idx_share_creating partial unique index; multiple ready or failed
// snapshots per share are explicitly allowed.
//
// State machine (enforced in UpdateSnapshotState): creating -> ready,
// creating -> failed, failed -> creating (retry). Any other transition
// returns models.ErrSnapshotStateConflict.
type SnapshotStore interface {
	// CreateSnapshot persists a new snapshot record.
	// If snap.ID is empty, a UUID is generated by the store.
	// If snap.State is empty, it defaults to models.StateCreating.
	// Returns the persisted ID.
	// Returns models.ErrSnapshotStateConflict if another in-flight
	// (state='creating') snapshot already exists for snap.ShareName.
	CreateSnapshot(ctx context.Context, snap *models.Snapshot) (string, error)

	// GetSnapshot returns a snapshot by (shareName, snapID).
	// Returns models.ErrSnapshotNotFound if no row matches.
	GetSnapshot(ctx context.Context, shareName, snapID string) (*models.Snapshot, error)

	// ListSnapshots returns ALL snapshots for shareName in ALL states
	// (creating, ready, failed), ordered by created_at DESC. Callers filter
	// by state as needed — HoldProvider filters to state='ready' itself.
	ListSnapshots(ctx context.Context, shareName string) ([]*models.Snapshot, error)
	...
	// Returns models.ErrSnapshotNotFound if no row matches.
	DeleteSnapshot(ctx context.Context, shareName, snapID string) error
