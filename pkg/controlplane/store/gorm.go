...
func New(config *Config) (*GORMStore, error) {
	...
	// Post-migration: fix portmapper_enabled for existing NFS adapter settings.
	// Same zero-value trap: ALTER TABLE ADD COLUMN sets bool to false, not true.
	if err := db.Exec(
		"UPDATE nfs_adapter_settings SET portmapper_enabled = ? WHERE portmapper_enabled = ? AND portmapper_port = ?",
		true, false, 0,
	).Error; err != nil {
