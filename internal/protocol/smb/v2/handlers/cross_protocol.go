...
// ============================================================================
// Cross-Protocol Status Code Selection
// ============================================================================

// statusForNLMConflict returns the appropriate SMB status code for an NLM lock conflict.
//
// Per MS-ERREF:
//   - STATUS_LOCK_NOT_GRANTED (0xC0000054): A requested lock operation could not be
//     performed because the requested range is locked by another process or client.
//
// This function always returns STATUS_LOCK_NOT_GRANTED for NLM lock conflicts.
// STATUS_SHARING_VIOLATION is NOT used because that's for share mode conflicts
// at file open time, not byte-range lock conflicts.
//
// Parameters:
//   - nlmLock: The conflicting NLM byte-range lock
//
// Returns:
//   - uint32: STATUS_LOCK_NOT_GRANTED (0xC0000054)
func statusForNLMConflict(nlmLock *lock.EnhancedLock) uint32 {
	// STATUS_LOCK_NOT_GRANTED is the appropriate code for byte-range lock conflicts
	// regardless of whether the lock is shared or exclusive
