...
// BACKCHANNEL_CTL is dispatched with no SEQUENCE-established session context
// (v41ctx == nil). No COMPOUND can deliver that combination -- a COMPOUND
// without SEQUENCE carries a single session-exempt operation and nothing else --
// so the handler's nil guard is driven through the dispatch table directly.
func TestBackchannelCtl_NoSession(t *testing.T) {
	...
	result := h.v41DispatchTable[types.OP_BACKCHANNEL_CTL](ctx, nil, bytes.NewReader(bctlArgs))
	if result.Status != types.NFS4ERR_OP_NOT_IN_SESSION {
		t.Errorf("status = %d, want NFS4ERR_OP_NOT_IN_SESSION (%d)",
			result.Status, types.NFS4ERR_OP_NOT_IN_SESSION)
	}
}
