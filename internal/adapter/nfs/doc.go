...
// the server speaks.
//
// It owns the framing, not the protocols: reading a record off a stream,
// reassembling the fragments a record is split across, rejecting a declared
// fragment size before it is used to size anything, and demultiplexing a
// backchannel reply that arrives interleaved with fore-channel calls.
//
