...
func TestSessionSetup_FullHandshake(t *testing.T) {
	t.Run("CompletesNTLMHandshake", func(t *testing.T) {
		...
		// Verify session was created. validNTLMAuthenticateMessage produces an
		// empty-username, zero-flags TYPE_3 — the anonymous path
		// (createAnonymousSession) handles this and marks the session IsNull,
		// not IsGuest. Previously routed through createGuestSessionWithID;
		// switching here mirrors MS-NLMP §3.3.2 anonymous handling and lets
		// smbtorture smb2.session.anon-encryption{1,2,3} flip — see #773.
...
func TestBuildAuthenticatedResponse_MICEmission(t *testing.T) {
	...
	t.Run("EmitsMICAndOmitsSupportedMech", func(t *testing.T) {
		...
		// FlagExtendedSecurity selects the NTLM2 (HMAC-MD5) MIC layout that
		// every modern SMB client negotiates. The SeqNum=0 assertion below
		// only holds in that layout; the legacy NTLM1 layout XORs RC4
		// keystream into bytes 12-15 (see TestComputeNTLMSSPMechListMICLegacy).
		...
		// SeqNum field (bytes 12-15) must be zero for SPNEGO MIC in the NTLM2
		// layout (HMAC-MD5).
		for i := 12; i < 16; i++ {
	...
	t.Run("NoMICWhenKeyAbsent", func(t *testing.T) {
		...
		result := h.buildAuthenticatedResponse(pending, nil, auth.FlagNTLM|auth.FlagKeyExch|auth.FlagExtendedSecurity, false)
