...
func TestSessionSetup_FullHandshake(t *testing.T) {
	t.Run("CompletesNTLMHandshake", func(t *testing.T) {
		...
		// Verify session was created. The minimal TYPE_3 generated by
		// validNTLMAuthenticateMessage has empty username + zero
		// NegotiateFlags, so SessionSetup routes through
		// createAnonymousSession and produces an IsNull session (anonymous
		// auth path) rather than the guest downgrade. Pre-#748 this used
		// to land in createGuestSessionWithID; updated to mirror the new
		// MS-NLMP-aligned anonymous handling that flips
		// smbtorture smb2.session.anon-signing1/2.
		session, ok := h.GetSession(sessionID)
		if !ok {
			t.Error("Session should be created after AUTHENTICATE")
		}
		if !session.IsNull {
			t.Error("Session should be marked as IsNull (anonymous authenticated)")
		}
		if session.IsGuest {
			t.Error("Session should not be marked as guest — anonymous is IsNull, not IsGuest")
		}
...
func TestBuildAuthenticatedResponse_MICEmission(t *testing.T) {
	...
	t.Run("EmitsMICAndOmitsSupportedMech", func(t *testing.T) {
		...
		// FlagExtendedSecurity selects the NTLM2 MIC layout (HMAC-MD5
		// based), which is what every real SMB client negotiates today.
		// Without it the MIC falls back to the legacy NTLM1 (CRC32 + RC4
		// keystream) layout used only by anonymous flows — see
		// computeNTLMSSPMechListMICLegacy in auth/ntlm.go.
		result := h.buildAuthenticatedResponse(pending, exportedSessionKey, auth.FlagNTLM|auth.FlagKeyExch|auth.FlagExtendedSecurity, false)
		...
		// SeqNum field (bytes 12-15) must be zero for SPNEGO MIC in the
		// NTLM2 layout — the legacy NTLM1 layout XORs keystream bytes here
		// so a fixed-zero assertion would only hold for the ext-session-
		// security path.
		for i := 12; i < 16; i++ {
