...
// both directions. Emit (SIDForUID/SIDForGID) advertises a file
...
// and the descriptor stays byte-for-byte what it is with no bridge installed.
...
// only", the behaviour with no bridge installed.
...
// SID when the bridge maps the uid. The SET_INFO gate uses this to distinguish
// a no-op owner re-set (accept) from a genuine unmappable chown (reject),
// independent of whether the reverse directory lookup is up.
...
// AD/SID principals that govern the share, not just the file's synthesized
// owner+SYSTEM descriptor. Pass nil to build a descriptor
...
//   - Only direct AD/SID grants (Who carries the "sid:" prefix) are surfaced.
...
// change instead of silently succeeding, while still treating a
