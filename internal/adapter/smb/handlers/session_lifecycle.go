...
// TreeConnection is an alias for types.TreeConnection. It is read by the
// handlers and by the tree-connect path that builds it, so it cannot sit on
// either side of that boundary. The alias keeps the unqualified spelling.
type TreeConnection = types.TreeConnection
