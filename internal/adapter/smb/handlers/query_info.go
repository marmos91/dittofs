...
func (h *Handler) buildFileInfoFromStore(authCtx *metadata.AuthContext, file *metadata.File, openFile *OpenFile, class types.FileInfoClass) ([]byte, error) {
	...
	case types.FileStandardInformation:
		...
		standardInfo.AllocationSize = effectiveAllocationSize(standardInfo.EndOfFile, openFile.GetRequestedAllocSize())
	...
	case types.FileNetworkOpenInformation:
		...
		networkInfo.AllocationSize = effectiveAllocationSize(networkInfo.EndOfFile, openFile.GetRequestedAllocSize())
		return EncodeFileNetworkOpenInfo(networkInfo), nil
...
func fileModeInformationValue(openFile *OpenFile) uint32 {
	return uint32(openFile.GetCreateOptions() & fileModeInformationModeMask)
}
