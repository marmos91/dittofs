...
// events for d before delivering them.
//
// It exists for callers outside this package that drive delivery explicitly
// through FlushAll instead of waiting on wall-clock, and must push the timer
// past their own lifetime to do it: with the production window armed, the
// time.AfterFunc goroutine can fire before FlushAll stops it and deliver the
// callback with no happens-before edge to the caller's assertion. On a fast
// machine FlushAll always wins and the race is invisible; on a contended
// Windows runner the timer wins often enough to flake. Inside this package the
// field is reachable directly, so this is the out-of-package spelling of that.
//
// decision: d is taken as given, with no floor. Only that upper use case has a
// caller, and a silent fallback to the default would be a surprise to whoever
// first passes something small on purpose. A d at or below zero arms
// time.AfterFunc to fire immediately and defeats the burst batching a single
// CHANGE_NOTIFY response depends on — clamp it once a caller wants a short
...
// WatcherCount for callers outside this package that assert on it. It has no
// production caller and is not meant to grow one.
