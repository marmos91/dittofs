...
// CacheInvalidator is the minimal cache-surface adapter helpers depend on for
// surgical post-transaction invalidation. It is defined here in package
// common rather than imported from pkg/block/engine so that adapter helpers
// remain decoupled from the concrete engine.Cache type — the engine's Cache
// type implements this interface implicitly via its InvalidateFile method.
//
// Contract:
...
//     a "drop everything for this payloadID" signal — used where a
//     destination's content changes wholesale.
