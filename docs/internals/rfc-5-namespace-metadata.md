...
| RFC 0 §9.2 (I8) conflicts are retried, not surfaced | Not met on Badger. `withTransaction` retries an SSI abort a fixed `maxTransactionRetries` times — 20 by default — then maps the exhausted conflict to `StoreError{Code: ErrConflict}`, which reaches the protocol layer as an I/O error. The bound is the constant, not the caller's deadline. The backoff is also deterministic: the term the code calls jitter is `attempt` milliseconds, so every loser of one conflict waits the same interval and re-collides on the next attempt — the comment at the site says so. Measured at one error per 200 commits with eight writers appending to a single file. This is the shared transaction wrapper, so it binds every record both this RFC and RFC 4 describe, not only namespace rows. | `store/badger/transaction.go:117`–`:215`, `:236` |
